...
{   
    ...
    set_sampling_frequency_and_gain(24000L, 30);

    EZDSP5535_SDCARD_init();
	
	printf("Playing recoreded audio using 24kHz sample rate");
    address = SD_CARD_START_ADDRESS;
    for(i = 0; i < n; i++)
    {
		EZDSP5535_SDCARD_read(address, BUFFER_SIZE*2, (Uint16*)OutputBuffer);
    ...
    set_sampling_frequency_and_gain(8000L, 30);

    EZDSP5535_SDCARD_init();
	
	printf("Playing recoreded audio using 8kHz sample rate");
    address = SD_CARD_START_ADDRESS;
    for(i = 0; i < n; i++)
    {
		EZDSP5535_SDCARD_read(address, BUFFER_SIZE*2, (Uint16*)OutputBuffer);
